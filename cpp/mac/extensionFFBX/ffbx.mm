...
namespace FFBX {
	...
	bool setOptionsOutput(const char * path, bool clearFolder, bool importAfter) {
		...
		_optionsOutput.path = [[[NSString stringWithUTF8String:path] stringByStandardizingPath] stringByAppendingString:@"/"];
	...
	void threadConversion() {
		...
		BOOL isDir;
		if (![fileManager fileExistsAtPath:_optionsOutput.path isDirectory:&isDir] && isDir) {
		...
		for (const auto & file3d : _sInputFilenames) {
			...
			// target filename
			NSString * fname = [[[[NSString stringWithUTF8String:file3d.c_str()] lastPathComponent] stringByDeletingPathExtension] stringByAppendingPathExtension:@"dae"];
			NSString * daefname = [NSString stringWithFormat:@"%@%@", _optionsOutput.path, fname];
			const char * newfname = [daefname UTF8String];

			// Initialize the DAE exporter.
			if (!lExporter->Initialize(newfname, -1, lSdkManager->GetIOSettings())) {
